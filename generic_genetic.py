...
class neuroevolution:
    ...
    def init_generation(self):
        ...
        for _ in range(0, self.population_size):
            model = Sequential()
            model.add(Dense(input_dim = self.layers[0], units = self.layers[1], activation='softmax', kernel_initializer='random_uniform', bias_initializer='random_uniform'))
            for i in range(2, len(self.layers)):
                model.add(Dense(units = self.layers[i], activation='softmax', kernel_initializer='random_uniform', bias_initializer='random_uniform'))
    ...
    def next_generation(self, fitness):
        ...
        print(fitness.shape, self.current_population.shape)
